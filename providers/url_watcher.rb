...
action :wait do
  ...
  if new_resource.match_string
    curl_validation_command = %(curl -k #{creds} --silent #{validation_url} | grep "#{new_resource.match_string}")
  else
    curl_validation_command = "curl -k #{creds} -o /dev/null --silent --head --write-out '%{http_code}' #{validation_url} | grep 200"
  end
