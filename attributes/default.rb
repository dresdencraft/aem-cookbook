...
default[:aem][:commands] = {
  replicators: {
    ...
    agent: {
      ...
      remove: {
        '5.4' => 'curl -k -u <%=local_user%>:<%=local_password%> -X DELETE <%=protocol%>://localhost:<%=local_port%>/etc/replication/agents.<%=server%>/<%=h%>'
      },
      list: {
        '5.4' => 'curl -k -u <%=local_user%>:<%=local_password%> <%=protocol%>://localhost:<%=local_port%>/etc/replication.infinity.json',
        '6.4' => 'curl -k -u <%=local_user%>:<%=local_password%> <%=protocol%>://localhost:<%=local_port%>/etc/replication/agents.<%=agent%>.infinity.json'
      }
    }
  },
  password: {
    '5.4' => 'curl -k -f --data rep:password=<%= password %> --user <%= admin_user %>:<%= admin_password %> <%=protocol%>://localhost:<%= port %><%= path %>/<%= user %>',
    '5.5' => 'curl -k -f --user <%= admin_user %>:<%= admin_password %> -F rep:password="<%= password %>" <%=protocol%>://localhost:<%= port %><%= path %>/<%= user %>.rw.html',
    '5.6' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -F rep:password="<%= password %>" -F :currentPassword="<%= admin_password %>" <%=protocol%>://localhost:<%= port %><%= path %>/<%= user %>.rw.html',
    '6.0' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -Fplain=<%= password %> -Fverify=<%= password %> -Fold=<%= admin_password %> -FPath=<%= path %>/<%= admin_user %> <%=protocol%>://localhost:<%= port %>/crx/explorer/ui/setpassword.jsp',
    '6.1' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -Fplain=<%= password %> -Fverify=<%= password %> -Fold=<%= admin_password %> -FPath=<%= path %> <%=protocol%>://localhost:<%= port %>/crx/explorer/ui/setpassword.jsp'
  },
  remove_user: {
    '5.5' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -FdeleteAuthorizable= <%=protocol%>://localhost:<%= port %><%= path %>/<%= user %>',
    '6.1' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -FdeleteAuthorizable=<%= user %> <%=protocol%>://localhost:<%= port %><%= path %>'
  },
  add_user: {
    '5.5' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -FcreateUser= -FauthorizableId=<%= user %> -Frep:password=<%= password %> -Fmembership=<%= group %> <%=protocol%>://localhost:<%= port %>/libs/granite/security/post/authorizables',
    '6.1' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -FcreateUser=1 -FauthorizableId=<%= user %> -Frep:password=<%= password %> <%=protocol%>://localhost:<%= port %>/libs/granite/security/post/authorizables.html'
  },
  add_user_to_group: {
    # Note that prior to 6.1 (possibly 6.0), the add_user command could add both the user AND add the user to a group.
    # That's no longer the case.
    '6.1' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -FaddMembers=<%= user %> <%=protocol%>://localhost:<%= port %><%= path %>.rw.userprops.html'
  },
  remove_user_from_group: {
    '6.1' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -FremoveMembers=<%= user %> <%=protocol%>://localhost:<%= port %><%= path %>.rw.userprops.html'
  },
  add_group: {
    '6.1' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -FauthorizableId=<%= group %> -FcreateGroup=1 <%=protocol%>://localhost:<%= port %>/libs/granite/security/post/authorizables.html'
  },
  remove_group: {
    '6.1' => 'curl -k -u <%= admin_user %>:<%= admin_password %> -FdeleteAuthorizable=1 <%=protocol%>://localhost:<%= port %><%= path %>.rw.html'
  }
}
...
default[:aem][:author] = {
  ...
  https_validation_urls: [
      'https://localhost:8443/libs/granite/core/content/login.html',
